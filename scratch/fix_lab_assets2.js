...
const tabLabEnd = html.indexOf('</div>', fearStart + 2000); // 2114 line's </div>
...
if (assetStart !== -1 && marketStart !== -1 && assetStart < marketStart) {
  ...
  // Recalculate tabLabEnd because html length changed
  const newTabLabEnd = html.indexOf('</div>', html.indexOf('<!-- 공포·탐욕 지수 섹션 -->') + 2000);
  
  // Insert assetBlock before newTabLabEnd
  html = html.substring(0, newTabLabEnd) + assetBlock + html.substring(newTabLabEnd);
}
...
// 4. Inject Modal & JS
// If it's already there (from my previous attempt that failed to inject properly), let's remove it or we just append it if not found.
