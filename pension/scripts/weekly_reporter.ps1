...
$marketSum = "오늘($lastTradingDateStr)의 전체 포트폴리오 자산은 전주 대비 <strong style='color:$colorTotal;'>$signHTMLTotal $fTotalChange 원</strong> 변동되었습니다."
...
$dateStr = (Get-Date).ToString("yyyy-MM-dd")
    $msg.Subject = "[자동보고] IRP 포트폴리오 주간 마감 리포트 ($dateStr)"
$msg.IsBodyHtml = $true
$msg.Body = $template
